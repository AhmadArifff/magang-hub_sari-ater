...
def get_jadwal_karyawan():
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        cursor.execute("""
            SELECT id, nama_karyawan, tanggal, kode_shift, jadwal_masuk, jadwal_pulang, keterangan
            FROM jadwal_karyawan
            ORDER BY id ASC
        """)
        rows = cursor.fetchall()

        # convert datetime/time ke string
        for row in rows:
            for key in ["tanggal", "jadwal_masuk", "jadwal_pulang"]:
...
def create_jadwal_karyawan():
    try:
        ...
        cursor.execute("""
            ...
            (nama_karyawan, tanggal, kode_shift, jadwal_masuk, jadwal_pulang, keterangan)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (
            data.get("nama_karyawan"),
            data.get("tanggal"),
            data.get("kode_shift"),
            data.get("jadwal_masuk"),
            data.get("jadwal_pulang"),
            data.get("keterangan")
        ))
        conn.commit()
        cursor.close()
...
# -----------------------
@app.route("/api/jadwal-karyawan/update/<int:id>", methods=["PUT"])
def update_jadwal_karyawan(id):
    try:
        data = request.get_json(force=True)
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM jadwal_karyawan WHERE id=%s", (id,))
        exists = cursor.fetchone()[0]
        if exists == 0:
            cursor.close()
            conn.close()
        ...
        cursor.execute("""
            UPDATE jadwal_karyawan
            SET nama_karyawan=%s, tanggal=%s, kode_shift=%s, jadwal_masuk=%s, jadwal_pulang=%s, keterangan=%s
            WHERE id=%s
        """, (
            data.get("nama_karyawan"),
            data.get("tanggal"),
...
# -----------------------
@app.route("/api/jadwal-karyawan/delete/<int:id>", methods=["DELETE"])
def delete_jadwal_karyawan(id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM jadwal_karyawan WHERE id=%s", (id,))
        conn.commit()
        cursor.close()
        conn.close()
