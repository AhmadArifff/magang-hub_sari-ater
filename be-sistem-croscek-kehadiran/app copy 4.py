...
def import_jadwal():
    ...
    cur.execute("SELECT id_karyawan, nik, nama,kategori FROM karyawan WHERE kategori = 'karyawan'")
    ...
    # Hapus jadwal lama bulan ini
    # cur.execute("DELETE FROM jadwal_karyawan WHERE kategori = 'dw' AND YEAR(tanggal)=%s AND MONTH(tanggal)=%s", (year, month))
    cur.execute("""
        DELETE jk
        FROM jadwal_karyawan jk
        INNER JOIN karyawan k
            ...
            queryfinal = """
                        ...
                        scan_data AS (
                            SELECT
                                ...
                                MAX(kk.tanggal_scan) AS scan_pulang
                            FROM kehadiran_karyawan kk
                            JOIN karyawan k ON k.id_absen = kk.pin  -- ✅ MATCH by PIN
                            LEFT JOIN used_scan_pulang_malam u
                                ON u.pin = kk.pin
                                AND u.tanggal_scan = kk.tanggal_scan
                            WHERE u.tanggal_scan IS NULL
